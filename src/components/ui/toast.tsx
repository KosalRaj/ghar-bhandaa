        ...
        {toasts.map((toast) => {
          ...
          return (
            <Toast.Root
              key={toast.id}
              className={cn(
                'absolute z-[calc(9999-var(--toast-index))] h-(--toast-calc-height) w-full select-none rounded-xl border bg-popover not-dark:bg-clip-padding text-popover-foreground shadow-lg/10 [transition:transform_.5s_cubic-bezier(.22,1,.36,1),opacity_.5s,height_.15s,background-color_.5s] before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] data-expanded:bg-popover dark:bg-popover dark:before:shadow-[0_-1px_--theme(--color-white/6%)]',
              ...
              toast={toast}
            >
              <Toast.Content className="pointer-events-auto flex items-center justify-between gap-2 overflow-hidden px-4 py-3 text-sm transition-opacity duration-250 data-behind:not-data-expanded:pointer-events-none data-behind:opacity-0 data-expanded:opacity-100">
                <div className="flex gap-2.5 items-start">
                  {Icon && (
                    <div
                      className="[&>svg]:h-5 [&>svg]:w-5 [&_svg]:pointer-events-none [&_svg]:shrink-0 mt-0.5"
                  ...
                  <div className="flex flex-col gap-0.5 text-left">
                    <Toast.Title
                      className="font-semibold text-sm text-[var(--sea-ink)]"
